...
const HeroSearch = () => {
  ...
  const handleSpecialistSelect = (specialist: string) => {
    router.push(`/doctors?specialization=${encodeURIComponent(specialist.toLowerCase().replace(/ /g, '-'))}&city=${selectedCity}`)
    setShowSpecialistDropdown(false)
  }

  const handleSearch = () => {
    if (searchText) {
      handleSpecialistSelect(searchText)
    } else {
      router.push(`/doctors?city=${selectedCity}`)
    }
  }
              ...
              <div className="flex items-center px-6 py-4">
                ...
                <div className="flex flex-col flex-1 text-left">
                  ...
                  <input
                    type="text"
                    value={searchText}
                    onFocus={() => { setShowSpecialistDropdown(true); setIsFocused(true) }}
